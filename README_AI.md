...
- вкладка «Память» в досье (`GET /api/lab/profiles/{id}/memory`): сущности и связи профильного графа LightRAG, а если экстракция ещё не завершена — сырые записи памяти из kv-хранилища; кнопка «🧹 Забыть всё» с подтверждением удаляет граф с диска (`DELETE /api/lab/profiles/{id}/memory`), обнуляет `memory_graph_id` и рассылает обновление инвентаря;
