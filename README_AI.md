...
### Экспорт, препринты и стоимость

- **Экспорт траекторий**: `GET /api/export/session/{id}?format=messages|sharegpt` и `GET /api/export/room/{id}` — JSONL-файлы для fine-tuning и публикации; кнопка `⬇ JSONL` в блоке отчёта;
- **Препринты**: кнопка `📄 Препринт` собирает документ «Постановка → Метод → Результаты → Открытые вопросы» по протоколу сессии. Пишется настроенной LLM памяти (с ретраями на rate-limit), при её недоступности — детерминированный шаблон из хроники; сохраняется в отчётах сессии;
- **Стоимость**: таблица `token_usage` копит оценку токенов по каждой реплике, реакции и обзору Хрономанта (кириллица ≈0.55 ток./символ); сводка — `GET /api/stats/tokens/{sessionId}` (по видам вызовов, моделям и раундам). OpenRouter дополнительно возвращает точный usage в тесте LLM памяти.

### Кросс-диалог и настройки в интерфейсе

- **@упоминания**: агент, адресованный через `@Имя` (парсится по именам активных участников), получает слово следующим — движок меняет его позицию местами с запланированным спикером; упоминания подсвечиваются золотым в чате;
- **реакции-перебивания**: между репликами с настраиваемой вероятностью другой агент вставляет ОДНУ короткую фразу (вес x3 у упомянутых, x0.15 у следующего по очереди); хранится как `agent_reaction` с тегом «реакция на @Имя»;
- **настройки в UI**: drawer `Провайдеры` → секция `Настройки системы`: LLM памяти (base URL/модель/ключ), вкл/выкл реакций, шанс перебивания. Хранятся в таблице БД `app_settings`, приоритет над env; кнопка «Тест LLM памяти» проверяет соединение реальным запросом;
- REST: `GET|PUT /api/settings` (секреты отдаются маской, пустой секрет = «не менять»), `POST /api/settings/test-memory-llm`;
- переменные окружения (`MEMORY_LLM_*`, `AGENT_HISTORY_LIMIT`, `RAG_CONTEXT_LIMIT`, `MEMORY_CONTEXT_LIMIT`, `CROSS_DIALOG_ENABLED`, `REACTION_CHANCE`) остаются fallback'ом, если в UI ничего не задано.
...
- вкладка «Память» в досье (`GET /api/lab/profiles/{id}/memory`): сущности и связи профильного графа LightRAG, а если экстракция ещё не завершена — сырые записи памяти из kv-хранилища;
- кнопка «🧹 Забыть всё» с подтверждением удаляет граф с диска (`DELETE /api/lab/profiles/{id}/memory`), обнуляет `memory_graph_id` и рассылает обновление инвентаря;
- кнопка «♻️ Пересобрать» (`POST /api/lab/profiles/{id}/memory/reindex`): дедуплицирует сырые записи графа (нормализация пробелов/регистра, свежие первыми, максимум 80) и заново прогоняет их через экстракцию сущностей с прогрессом по WebSocket (`memory_reindex_progress`);
- редактирование профиля по-прежнему в Инвентаре — Лаборатория пока read-only.

#### Память: конфигурация и best practices

- LightRAG: `lightrag-hku>=1.5.6`; функция эмбеддингов обязана возвращать numpy-массив (иначе пайплайн вставки падает на первом чанке);
- модель извлечения сущностей настраивается независимо от моделей дебатов (любой OpenAI-совместимый endpoint), переменные `backend/.env`:

| Переменная | По умолчанию | Назначение |
| --- | --- | --- |
| `MEMORY_LLM_BASE_URL` | `http://localhost:11434/v1` | Endpoint для экстракции памяти |
| `MEMORY_LLM_MODEL` | `gemma4:31b-cloud` | Модель экстракции |
| `MEMORY_LLM_API_KEY` | пусто | Bearer-ключ (DeepSeek/OpenRouter/…) |

- экономия токенов на реплику агента: `AGENT_HISTORY_LIMIT` (свежих сообщений целиком, по умолчанию 12 — остальное покрыто хроникой), `RAG_CONTEXT_LIMIT` (4000 символов), `MEMORY_CONTEXT_LIMIT` (2000 символов);
- сбои записи памяти больше не глотаются молча: пишутся в лог с трейсбеком.
